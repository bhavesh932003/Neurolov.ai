...
export default function NeuroImageGenerator() {
  ...
  // New state for tracking image generations, user credits, and image count based on plan.
  ...
  // Fetch image counts based on plan and check for plan change.
  useEffect(() => {
    if (user) {
      const fetchImageCount = async () => {
        ...
        if (error) {
          ...
          // If plan changed, reset the corresponding image count to 0.
          if (previousPlanRef.current && previousPlanRef.current !== data.plan) {
            let columnToReset = "";
            if (data.plan === "free") {
              columnToReset = "image_count_free";
            } else if (data.plan === "pro") {
              columnToReset = "image_count_pro";
            } else if (data.plan === "ultimate") {
              columnToReset = "image_count_ultimate";
            }
            await supabase.from("profiles").update({ [columnToReset]: 0 }).eq("id", user?.id);
            setImageCount(0);
          } else {
            // Otherwise, set imageCount based on the current plan.
            if (data.plan === "free") {
              setImageCount(data.image_count_free || 0);
            } else if (data.plan === "pro") {
              setImageCount(data.image_count_pro || 0);
            } else if (data.plan === "ultimate") {
              setImageCount(data.image_count_ultimate || 0);
            }
          }
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Set minimum generation delay based on user's plan:
      // free: 10 sec, pro: 6 sec, ultimate: 0 sec
      ...
      // Ensure a minimum delay before showing the image.
      const elapsed = Date.now() - startTime;
      if (elapsed < minGenerationDelay) {
        await new Promise((resolve) =>
          setTimeout(resolve, minGenerationDelay - elapsed)
        );
      }
      ...
      if (data.images?.[0]) {
        ...
        // Update the image count in the appropriate column based on plan.
        let columnName = "";
        if (plan === "free") {
          columnName = "image_count_free";
        } else if (plan === "pro") {
          columnName = "image_count_pro";
        } else if (plan === "ultimate") {
          columnName = "image_count_ultimate";
        }
        const newImageCount = imageCount + 1;
        setImageCount(newImageCount);
        await supabase.from("profiles").update({ [columnName]: newImageCount }).eq("id", user?.id);

        // Determine if this generation should be free based on plan rules.
  ...
  // Function to update quest progress via API.
